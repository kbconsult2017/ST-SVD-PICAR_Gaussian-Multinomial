# Gaussian simulation study: ST SVD-PICAR
...
# Output folder
out_dir <- "C:/Users/Admin/OneDrive - University of Nebraska Medical Center/Desktop/svd-continuous"
...
pdf_file_for_K <- function(K) {
  file.path(out_dir, paste0("SVD_PICAR_n5000_constructed_bases_", K,
                            "_ALL_OUTPUTS_", timestamp, ".pdf"))
}

# Global MCMC PDF
pdf_file_mcmc <- file.path(out_dir, "SVD_PICAR_n5000_constructed_bases_200_600_ALL_OUTPUTS_MCMC.pdf")
...
# Temporal basis
tBase_train <- bs(train_data$year_mo, df=num_t_basis, intercept=FALSE)
tBase_test  <- bs(test_data$year_mo,  df=num_t_basis, intercept=FALSE)
...
svd_truth <- rsvd::rsvd(ST_train_full_GLOBAL, k=K_truth)
U_truth <- svd_truth$u[, 1:K_truth, drop=FALSE]
D_truth <- diag(svd_truth$d[1:K_truth])
V_truth <- svd_truth$v[, 1:K_truth, drop=FALSE]

B_train_truth <- U_truth %*% D_truth
B_test_truth  <- ST_test_full_GLOBAL %*% V_truth
...
MQM_truth_reduced <- as.matrix(t(V_truth) %*% MQM_full_GLOBAL %*% V_truth)
MQM_truth_reduced <- 0.5*(MQM_truth_reduced + t(MQM_truth_reduced)) + diag(1e-6, nrow(MQM_truth_reduced))

Qdelta_true <- tau_true * MQM_truth_reduced + diag(1e-8, K_truth)
...
for (K in constructed_K_values) {
  ...
  pdf_text_page(paste0("SVD–PICAR (continuous, n=5000) — K = ", K),
...
# FULL-BASIS -> SVD COMPRESS TO K

  max_combined_rank <- min(K, P_full)

  p_max <- num_eigen_full
  q_max <- num_t_basis
...
pdf_text_page("02 — RANK SELECTION (ALL Kbasis) — CONTINUOUS SVD–PICAR",
              c(paste0("Created: ", format(Sys.time(), "%Y-%m-%d %H:%M:%S")),
...
if (nrow(best_ranksel_byK) > 0) {
  ...
  print(
    ggplot(best_ranksel_byK, aes(x=Constructed_ST_Basis, y=Min_MSPE)) +
      geom_line() + geom_point(size=2) +
      theme_minimal() +
      labs(title="Best (min MSPE) vs Kbasis", x="K (constructed ST basis)", y="Min MSPE")
  )
  ...
  print(
    ggplot(best_ranksel_byK, aes(x=Constructed_ST_Basis, y=Best_Rank)) +
      geom_line() + geom_point(size=2) +
      theme_minimal() +
      labs(title="Selected best rank vs Kbasis", x="K (constructed ST basis)", y="Best Rank")
  )
} else {
...
pdf_text_page("03 — COEFFICIENTS vs RANK (ALL Kbasis) — CONTINUOUS SVD–PICAR",
              c(paste0("Created: ", format(Sys.time(), "%Y-%m-%d %H:%M:%S")),
...
pdf_text_page("04 — BEST METRICS vs Kbasis — CONTINUOUS SVD–PICAR",
              c(paste0("Created: ", format(Sys.time(), "%Y-%m-%d %H:%M:%S")),
...
if (nrow(best_ranksel_byK) > 0) {
  ...
  print(
    ggplot(bm, aes(x=Constructed_ST_Basis, y=Min_MSPE)) +
      geom_line() + geom_point(size=2) +
      theme_minimal() +
      labs(title="Best MSPE vs Kbasis", x="K (constructed ST basis)", y="Min MSPE")
  )
  ...
  print(
    ggplot(bm, aes(x=Constructed_ST_Basis, y=Best_Rank)) +
      geom_line() + geom_point(size=2) +
      theme_minimal() +
      labs(title="Best rank vs Kbasis", x="K (constructed ST basis)", y="Best Rank")
  )
  ...
  print(
    ggplot(ESS_summary_byK, aes(x=Constructed_ST_Basis, y=ESSps_mean_all)) +
      ...
      labs(title="Best-rank mean ESS/sec (ALL monitored parameters) vs K",
           x="K (constructed ST basis)", y="Mean ESS/sec (all params)")
  )
  ...
  print(
    ggplot(ESS_summary_byK, aes(x=Constructed_ST_Basis, y=ESSps_min_all)) +
      ...
      labs(title="Best-rank MIN ESS/sec (ALL monitored parameters) vs K",
           x="K (constructed ST basis)", y="Min ESS/sec (all params)")
  )
  ...
  print(
    ggplot(ESS_summary_byK, aes(x=Constructed_ST_Basis, y=ESSps_mean_beta12)) +
      ...
      labs(title="Best-rank mean ESS/sec (beta1,beta2) vs K",
           x="K (constructed ST basis)", y="Mean ESS/sec (beta1,beta2)")
  )
}
...
save(
  ...
  file = file.path(out_dir, paste0("All_Objects_Continuous_SVD_PICAR_FAST_", timestamp, ".RData"))
)
...
# 12) NEW ANALYSIS PDF (Option 1 = fully defensible)

pdf_new_analysis <- file.path(out_dir, "08_SVD_PICAR_Oversmoothing_and_MeshDecoupling.pdf")
csv_mesh_sens    <- file.path(out_dir, "MeshDensity_Sensitivity_SVD_PICAR.csv")
csv_rank_sweep   <- file.path(out_dir, "Roughness_ByRank_SVD_PICAR_Gaussian.csv")
...
pdf_text_page <- function(title, bullets, cex_title=1.1, cex_body=0.95) {
  grid.newpage()
...
truth_ok <- TRUE
if (!exists("V_truth") || !exists("delta_true")) {
  truth_ok <- FALSE
  warning("V_truth or delta_true not found; roughness ratio-to-truth will be NA.")
}
...
if (truth_ok) {
  t_rowv <- bs_eval_like_train(t_star, tBase_train)
  temporal_meshv <- matrix(rep(t_rowv, each=nrow(mesh$loc)), nrow=nrow(mesh$loc), byrow=FALSE)
  ST_mesh_fullv <- make_st_scores(mBase_full, temporal_meshv)
  ST_mesh_truthK <- as.matrix(ST_mesh_fullv %*% V_truth)
...
cat("\n[NEW ANALYSIS] Option 1: fitting MANY ranks for each K (SVD-PICAR Gaussian)...\n")
...
for (K_map in K_vals_sweep) {
  ...
  for (rnk in r_grid) {
    ...
    if (length(delta_cols) != rnk) {
      warning(paste0("Delta columns mismatch at K=",K_map," r=",rnk,". Found ",
                     length(delta_cols)," expected ",rnk))
    }
...
cat("\n[NEW ANALYSIS] Mesh-density sensitivity experiment...\n")
...
for (me in max_edge_grid) {
  ...
  mBase_tmp <- build_moran_basis(W_tmp, num_eigen_full)
  ...
  svd_tmp <- rsvd::rsvd(ST_tr_full_tmp, k = K_use_tmp)

  U_tmp <- svd_tmp$u[, 1:K_use_tmp, drop=FALSE]
  D_tmp <- diag(svd_tmp$d[1:K_use_tmp])
  V_tmp <- svd_tmp$v[, 1:K_use_tmp, drop=FALSE]
  ...
  r_sens <- min(as.integer(r_sens_target), K_use_tmp)
...
pdf_text_page(
  "NEW ANALYSIS — SVD–PICAR (Gaussian): Oversmoothing (maps+roughness) + Mesh-density decoupling",
  c(
...
pdf_text_page(
  "PART 1 — Oversmoothing vs fine-scale: rank-sweep roughness curves (Option C)",
  c(
...
# Part 1B (UPDATED): Latent field panels at t* for EACH K at best MSPE
pdf_text_page(
...
p_panels <- ggplot(latent_byK, aes(x=lon, y=lat, color=u_hat)) +
  ...
  theme_minimal() +
  labs(
    title    = "Gaussian SVD–PICAR: posterior-mean latent field u_hat(s,t*) at best MSPE rank per K",
