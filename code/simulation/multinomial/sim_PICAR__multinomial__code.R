# Multinomial simulation study: ST PICAR (no SVD)
...
# Output folder
out_dir <- "C:/Users/Admin/OneDrive - University of Nebraska Medical Center/Desktop/PICAR _mutinomial"
...
required_packages <- c(
  ...
  "reshape2","dplyr","tidyr","nimble","coda","VGAM"
)
...
B_train_truth_raw <- full_ST$ST_train_full[, 1:Kbasis_truth, drop=FALSE]
B_test_truth_raw  <- full_ST$ST_test_full[,  1:Kbasis_truth, drop=FALSE]
MQM_truth         <- full_ST$MQM_full[1:Kbasis_truth, 1:Kbasis_truth, drop=FALSE]

Bc_truth <- center_basis_train_test(B_train_truth_raw, B_test_truth_raw)
B_train_truth <- Bc_truth$B_train
B_test_truth  <- Bc_truth$B_test

r_true <- ncol(B_train_truth)
tau_true <- 2
Qdelta <- tau_true * MQM_truth + diag(1e-8, r_true)
Rchol <- chol(Qdelta)
...
# 11) LOOP OVER Kbasis: BUILD BASIS (TRUNCATION) -> VGAM RANK SELECTION
rank_all <- data.frame()
...
for (Kbasis in Kbasis_vals) {
  ...
  cat("\n====================================================\n")
  cat("Kbasis =", Kbasis, "(# ST basis columns USED; NO SVD)\n")
  cat("====================================================\n")

  per_dir <- file.path(out_dir, "PerKbasis", paste0("Kbasis_", Kbasis))
  dir.create(per_dir, showWarnings = FALSE, recursive = TRUE)

  B_train_raw <- full_ST$ST_train_full[, 1:Kbasis, drop=FALSE]
  B_test_raw  <- full_ST$ST_test_full[,  1:Kbasis, drop=FALSE]
  MQM_Kbasis  <- full_ST$MQM_full[1:Kbasis, 1:Kbasis, drop=FALSE]

  Bc <- center_basis_train_test(B_train_raw, B_test_raw)
...
# OPTIONAL: NIMBLE MCMC FOR THIS Kbasis AT ITS BEST RANK
  if (RUN_MCMC_EACH_KBASIS) {
    ...
    MQM_rank <- MQM_Kbasis[1:r_use, 1:r_use, drop=FALSE]
...
if (RUN_CV_BEST_KBASIS) {
  ...
  B_train_best_raw <- full_ST$ST_train_full[, 1:bestK, drop=FALSE]
  B_test_best_raw  <- full_ST$ST_test_full[,  1:bestK, drop=FALSE]

  Bc_best <- center_basis_train_test(B_train_best_raw, B_test_best_raw)
...
save(
  ...
  file = file.path(out_dir, "All_Objects_Multinomial_PICAR_NoSVD_Kbasis200_600.RData")
)
...
pdf08_file        <- file.path(out_dir, "08_PICAR_Multinomial_Oversmoothing_and_MeshDecoupling.pdf")
csv_meshsens_file <- file.path(out_dir, "MeshDensity_Sensitivity_PICAR_Multinomial.csv")
csv_ranksweep     <- file.path(out_dir, "Roughness_ByRank_PICAR_Multinomial.csv")
...
build_kbasis_cached <- function(Kbasis, full_ST) {
  Btr_raw <- full_ST$ST_train_full[, 1:Kbasis, drop=FALSE]
  Bte_raw <- full_ST$ST_test_full[,  1:Kbasis, drop=FALSE]
  MQM_K   <- full_ST$MQM_full[1:Kbasis, 1:Kbasis, drop=FALSE]

  Bc <- center_basis_train_test(Btr_raw, Bte_raw)
...
compute_u_vertices_at_t_cached <- function(kbc, r_use, delta_mean, t_star,
  ...
  B_vert_full <- ST_vert_full[, 1:kbc$Kbasis, drop=FALSE]
  B_vert_full <- sweep(B_vert_full, 2, kbc$center, "-")

  u_vert <- as.vector(B_vert_full[, 1:r_use, drop=FALSE] %*% as.numeric(delta_mean[1:r_use]))
  u_vert
}
...
cat("\n[OPTION 1 | MULTINOMIAL PICAR (NO SVD)] Rank sweep starting...\n")
...
for (Kb in Kbasis_sweep) {
  ...
  for (rnk in r_grid) {
    ...
    MQM_rank <- kbc$MQM_K[1:r_use, 1:r_use, drop=FALSE]
...
if (RUN_MESH_PILOT) {
  ...
  for (me0 in maxedge_grid) {
    ...
    bt <- system.time({
      ...
      Btr_i_raw <- full_ST_i$ST_train_full[, 1:Kbasis_pilot, drop=FALSE]
      Bte_i_raw <- full_ST_i$ST_test_full[,  1:Kbasis_pilot, drop=FALSE]
      MQM_i_K   <- full_ST_i$MQM_full[1:Kbasis_pilot, 1:Kbasis_pilot, drop=FALSE]

      Bc_i <- center_basis_train_test(Btr_i_raw, Bte_i_raw)
      B_train_full_i <- Bc_i$B_train

      r_use_i <- min(rank_pilot, ncol(B_train_full_i))
      B_train_rank_i <- B_train_full_i[, 1:r_use_i, drop=FALSE]
      MQM_rank_i <- MQM_i_K[1:r_use_i, 1:r_use_i, drop=FALSE]
...
pdf_text_page(
  "08 — PICAR (Multinomial, NO SVD): Option 1 Rank Sweep + Mesh-density Decoupling",
  c(
...
p_panels <- ggplot(latent_byK, aes(x=lon, y=lat, color=u_hat)) +
  ...
  theme_minimal() +
  labs(
    title    = "Multinomial PICAR (NO SVD): posterior-mean latent field u(v,t*) at best rank per Kbasis",
