# Multinomial data application: ST PICAR (no SVD)
...
# Output folder
out_dir <- "C:/Users/Admin/OneDrive - University of Nebraska Medical Center/Desktop/d_picar_multinomial"
...
required_packages <- c(
  ...
  "reshape2","dplyr","tidyr","nimble","coda","VGAM","gridExtra","grid"
)
...
# 5) NO SVD: BUILD "Kbasis" BY TRUNCATING FULL ST BASIS + MATCHING MQM
build_Kbasis_trunc <- function(ST_train_full, ST_test_full, MQM_full, Kbasis) {
  Kb <- min(as.integer(Kbasis), ncol(ST_train_full))
  B_train <- ST_train_full[, 1:Kb, drop=FALSE]
  B_test  <- ST_test_full[,  1:Kb, drop=FALSE]

  colnames(B_train) <- paste0("STBase_", 1:ncol(B_train))
  colnames(B_test)  <- paste0("STBase_", 1:ncol(B_test))

  MQM_reduced <- MQM_full[1:Kb, 1:Kb, drop=FALSE]
  MQM_reduced <- 0.5*(MQM_reduced + t(MQM_reduced)) + diag(1e-6, nrow(MQM_reduced))

  list(B_train = B_train, B_test = B_test, MQM_reduced = MQM_reduced)
}
...
# 11) LOOP OVER Kbasis: TRUNCATE BASIS -> VGAM RANK SELECTION -> MCMC @
rank_all <- data.frame()
...
for (Kbasis in Kbasis_vals) {
  ...
  cat("\n====================================================\n")
  cat("Kbasis =", Kbasis, "(TRUNCATED full ST basis size; NO SVD)\n")
  cat("====================================================\n")

  per_dir <- file.path(out_dir, "PerKbasis", paste0("Kbasis_", Kbasis))
  dir.create(per_dir, showWarnings = FALSE, recursive = TRUE)

  kb <- build_Kbasis_trunc(full_ST$ST_train_full, full_ST$ST_test_full, full_ST$MQM_full,
                           Kbasis=Kbasis)
...
# NIMBLE MCMC FOR THIS Kbasis AT ITS BEST RANK
  if (RUN_MCMC_EACH_KBASIS) {
    ...
    B_train_raw <- as.matrix(kb$B_train)
    B_test_raw  <- as.matrix(kb$B_test)
    K_used <- ncol(B_train_raw)

    r_use <- as.integer(best_rank)
    r_use <- max(1L, min(r_use, K_used))

    B_train_rank <- B_train_raw[, 1:r_use, drop = FALSE]
    B_test_rank  <- B_test_raw[,  1:r_use, drop = FALSE]
...
if (RUN_CV_BEST_KBASIS) {
  ...
  kb_best <- build_Kbasis_trunc(full_ST$ST_train_full, full_ST$ST_test_full, full_ST$MQM_full,
                                Kbasis=bestK)
...
save(list = obj_list, file = file.path(out_dir, "All_Objects_Multinomial_PICAR_NO_SVD.RData"))
...
pdf08_file        <- file.path(out_dir, "08_PICAR_Multinomial_Oversmoothing_and_MeshDecoupling_NO_SVD.pdf")
csv_meshsens_file <- file.path(out_dir, "MeshDensity_Sensitivity_PICAR_Multinomial_NO_SVD.csv")
csv_ranksweep     <- file.path(out_dir, "Roughness_ByRank_PICAR_Multinomial_NO_SVD.csv")
...
compute_u_vertices_at_t_cached <- function(kbc, r_use, delta_mean, t_star,
  ...
  Kb <- min(as.integer(kbc$Kbasis), ncol(ST_vert_full))
  B_vert <- ST_vert_full[, 1:Kb, drop=FALSE]

  B_vert <- sweep(B_vert, 2, kbc$center[1:ncol(B_vert)], "-")

  r_use <- min(as.integer(r_use), ncol(B_vert))
...
if (RUN_MESH_PILOT) {
  ...
  for (me0 in maxedge_grid) {
    ...
    bt <- system.time({
      ...
      kb_i <- build_Kbasis_trunc(full_ST_i$ST_train_full, full_ST_i$ST_test_full, full_ST_i$MQM_full,
                                 Kbasis=Kbasis_pilot)
