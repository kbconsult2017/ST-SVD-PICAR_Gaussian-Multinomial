# Gaussian data application: ST PICAR (no SVD)
...
out_dir <- "C:/Users/Admin/OneDrive - University of Nebraska Medical Center/Desktop/d_picar_continuous"
...
pdf_file_for_K <- function(K) {
  file.path(out_dir, paste0("PICAR_NO_SVD_REALDATA_", n_label,
                            "_constructed_bases_", K,
                            "_ALL_OUTPUTS_", timestamp, ".pdf"))
}

pdf_file_mcmc <- file.path(out_dir, paste0("PICAR_NO_SVD_REALDATA_", n_label,
...
for (K in constructed_K_values) {
  ...
  pdf_text_page(paste0("PICAR (continuous, REAL DATA, NO SVD) — K = ", K),
...
# NO SVD: Directly truncate the FULL tensor-product ST basis to K columns
  max_combined_rank <- min(K, P_full)

  ST_train_reduced <- as.matrix(ST_train_full_GLOBAL[, 1:max_combined_rank, drop=FALSE])
  ST_test_reduced  <- as.matrix(ST_test_full_GLOBAL[,  1:max_combined_rank, drop=FALSE])
  ...
  MQM_full <- as.matrix(MQM_full_GLOBAL[1:max_combined_rank, 1:max_combined_rank, drop=FALSE])
  ...
  if (RUN_MCMC_BLOCK) {
    ...
    for (rnk in mcmc_rank_grid) {
      ...
      MQM_reduced <- as.matrix(MQM_full[1:rnk, 1:rnk, drop=FALSE])
...
pdf_text_page("02 — RANK SELECTION (ALL Kbasis) — CONTINUOUS PICAR (REAL DATA, NO SVD)",
              c(paste0("Created: ", format(Sys.time(), "%Y-%m-%d %H:%M:%S")),
...
pdf_text_page("03 — COEFFICIENTS vs RANK (ALL Kbasis) — CONTINUOUS PICAR (REAL DATA, NO SVD)",
              c(paste0("Created: ", format(Sys.time(), "%Y-%m-%d %H:%M:%S")),
...
pdf_text_page("04 — BEST METRICS vs Kbasis — CONTINUOUS PICAR (REAL DATA, NO SVD)",
              c(paste0("Created: ", format(Sys.time(), "%Y-%m-%d %H:%M:%S")),
...
pdf_text_page("GLOBAL ESS SUMMARY (BEST RANK PER K) — REAL DATA (NO SVD)",
              c(paste0("Created: ", format(Sys.time(), "%Y-%m-%d %H:%M:%S")),
...
pdf_text_page("BEST-RANK RUNTIME vs ESS/sec (REAL DATA, NO SVD)",
              c(paste0("Created: ", format(Sys.time(), "%Y-%m-%d %H:%M:%S")),
...
pdf_text_page("MCMC SUMMARY + COMBINED CURVES (ALL K) — REAL DATA (NO SVD)",
              c(paste0("Created: ", format(Sys.time(), "%Y-%m-%d %H:%M:%S")),
...
save(
  ...
  file = file.path(out_dir, paste0("All_Objects_Continuous_PICAR_NO_SVD_REALDATA_FAST_", n_label, "_", timestamp, ".RData"))
)
...
# 12) NEW ANALYSIS PDF (PICAR (NO SVD) — REAL DATA VERSION)

pdf_new_analysis <- file.path(out_dir, "08_PICAR_Oversmoothing_and_MeshSensitivity_REALDATA.pdf")
csv_mesh_sens    <- file.path(out_dir, "MeshDensity_Sensitivity_PICAR_REALDATA.csv")
csv_rank_sweep   <- file.path(out_dir, "Roughness_ByRank_PICAR_Gaussian_REALDATA.csv")
...
# Helper: choose (p_use, q_use) so p_use*q_use <= K_map
choose_pq_for_K <- function(K_map, p_full, q_full) {
  p_use <- min(p_full, max(1L, floor(sqrt(K_map))))
  q_use <- min(q_full, max(1L, ceiling(K_map / p_use)))
  while (p_use*q_use > K_map && p_use > 1L) p_use <- p_use - 1L
  K_used <- p_use * q_use
  list(p_use=p_use, q_use=q_use, K_used=K_used)
}

# Helper: compute PICAR latent field u on mesh vertices at fixed time t*
compute_u_mesh_at_tstar_PICAR <- function(mesh_obj, mBase_full, tBase_train,
                                          p_use, q_use, rnk, delta_bar, t_star) {
  ...
  t_row <- t_row[, 1:q_use, drop=FALSE]

  Nmesh <- nrow(mesh_obj$loc)

  temporal_mesh <- matrix(rep(t_row, each=Nmesh), nrow=Nmesh, byrow=FALSE)

  spatial_mesh <- as.matrix(mBase_full[, 1:p_use, drop=FALSE])
  ST_mesh_full <- make_st_scores(spatial_mesh, temporal_mesh)

  M_mesh <- ST_mesh_full[, 1:rnk, drop=FALSE]
...
make_rank_sweep_grid <- function(K_used) {
  ...
  base <- unique(sort(base))
  base[base >= 1L & base <= K_used]
}

# Storage for rank-sweep results
rank_sweep_res <- data.frame()

cat("\n[NEW ANALYSIS — REAL DATA] Part 1: fitting MANY ranks for each K (Gaussian PICAR; NO SVD)...\n")

p_full <- ncol(mBase_full)
q_full <- ncol(tBase_full_train)
spatial_full_train_ALL <- as.matrix(AMat_train %*% mBase_full)
...
for (K_map in K_vals_sweep) {
  ...
  pq <- choose_pq_for_K(K_map, p_full = p_full, q_full = q_full)
  p_use <- pq$p_use
  q_use <- pq$q_use
  K_used <- pq$K_used

  cat("      Using (p_use, q_use, K_used) = (", p_use, ", ", q_use, ", ", K_used, ")\n", sep="")

  spatial_tr <- spatial_full_train_ALL[, 1:p_use, drop=FALSE]
  temporal_tr <- tBase_full_train[, 1:q_use, drop=FALSE]
  ST_train_K <- make_st_scores(spatial_tr, temporal_tr)

  MQM_space_sub <- MQM_space_full[1:p_use, 1:p_use, drop=FALSE]
  MQM_space_sub <- 0.5*(MQM_space_sub + t(MQM_space_sub)) + diag(1e-6, nrow(MQM_space_sub))
  MQM_full_K <- as.matrix(kronecker(MQM_space_sub, Diagonal(q_use)))
  MQM_full_K <- 0.5*(MQM_full_K + t(MQM_full_K)) + diag(1e-6, nrow(MQM_full_K))
  ...
  for (rnk in r_grid) {
    ...
    M_train <- ST_train_K[, 1:rnk, drop=FALSE]
    MQM_red <- MQM_full_K[1:rnk, 1:rnk, drop=FALSE]
    MQM_red <- 0.5*(MQM_red + t(MQM_red)) + diag(1e-6, nrow(MQM_red))

    out_fit <- run_picar_mcmc_one_rank(
      y_train       = as.numeric(response_train),
      X_matrix      = as.matrix(covariates_train),
      M_matrix      = as.matrix(M_train),
      MQM_reduced   = as.matrix(MQM_red),
...
# Part 2) Mesh-density sensitivity
...
cat("\n[NEW ANALYSIS — REAL DATA] Part 2: mesh-density sensitivity experiment (PICAR; NO SVD)...\n")
...
for (me in max_edge_grid) {
  ...
  mBase_tmp_full <- build_moran_basis(W_tmp, num_eigen_tmp)

  q_full_tmp <- ncol(tBase_full_train)

  pq_tmp <- choose_pq_for_K(K_sens, p_full = ncol(mBase_tmp_full), q_full = q_full_tmp)
  p_use_tmp <- pq_tmp$p_use
  q_use_tmp <- pq_tmp$q_use
  K_used_tmp <- pq_tmp$K_used

  spatial_tr_tmp  <- as.matrix(A_tr_tmp %*% mBase_tmp_full[, 1:p_use_tmp, drop=FALSE])
  temporal_tr_tmp <- tBase_full_train[, 1:q_use_tmp, drop=FALSE]
  ST_tr_tmp <- make_st_scores(spatial_tr_tmp, temporal_tr_tmp)

  MQM_space_tmp <- as.matrix(t(mBase_tmp_full[,1:p_use_tmp,drop=FALSE]) %*% Q_tmp %*% mBase_tmp_full[,1:p_use_tmp,drop=FALSE])
  MQM_space_tmp <- 0.5*(MQM_space_tmp + t(MQM_space_tmp)) + diag(1e-6, nrow(MQM_space_tmp))
  MQM_full_tmp  <- as.matrix(kronecker(MQM_space_tmp, Diagonal(q_use_tmp)))
  MQM_full_tmp  <- 0.5*(MQM_full_tmp + t(MQM_full_tmp)) + diag(1e-6, nrow(MQM_full_tmp))

  r_sens <- min(as.integer(r_sens_target), as.integer(K_used_tmp))
  r_sens <- max(1L, r_sens)

  M_tr_tmp   <- as.matrix(ST_tr_tmp[, 1:r_sens, drop=FALSE])
  MQM_red_tmp <- as.matrix(MQM_full_tmp[1:r_sens, 1:r_sens, drop=FALSE])
...
pdf_text_page_12(
  "NEW ANALYSIS — PICAR (Gaussian, REAL DATA; NO SVD): Oversmoothing (maps+roughness) + Mesh-density sensitivity",
  c(
    paste0("Created: ", format(Sys.time(), "%Y-%m-%d %H:%M:%S")),
    paste0("Time slice: t* = ", round(t_star, 4)),
    "Part 1: Fit many ranks r for each requested K_map; compute u_hat(s,t*) and edge-roughness R(u).",
    "Part 1 Maps: One panel per K_map at that K_map's best MSPE rank (common color scale).",
    "Part 2: Vary mesh density; measure basis build time and MCMC time/iter (fixed K_sens, fixed r).",
...
pdf_text_page_12(
  "PART 1 — Oversmoothing vs fine-scale: rank-sweep roughness curves (REAL DATA; PICAR; NO SVD)",
  c(
    "For each K_map and rank r, we refit the Gaussian PICAR model and reconstruct the posterior-mean u_hat(s,t*).",
    ...
    "We also normalize within each K_map by the *largest rank fitted* (ratio-to-reference), to show relative smoothing."
  )
)
...
if (nrow(rank_sweep_res) > 0) {
  ...
  p_abs <- ggplot(rank_sweep_res, aes(x=rank, y=R_hat)) +
    ...
    theme_minimal() +
    labs(title="Absolute edge roughness R_hat vs rank r",
         x="Rank r", y="R_hat")
  print(p_abs)
  ...
  p_sf <- ggplot(rank_sweep_res, aes(x=rank, y=sf_hat)) +
    ...
    facet_wrap(~K_map, scales="free_x") +
    theme_minimal() +
    labs(title="Scale-free roughness sqrt(R_hat)/sd(u_hat) vs rank r",
         x="Rank r", y="sqrt(R_hat)/sd(u_hat)")
  print(p_sf)
  ...
  if (all(c("R_ratio_ref","sf_ratio_ref") %in% names(rank_sweep_res))) {
    ...
    p1 <- ggplot(rank_sweep_res, aes(x=rank, y=logRratio_ref)) +
      ...
      facet_wrap(~K_map, scales="free_x") +
      theme_minimal() +
      labs(title="log10( R_hat / R_ref ) vs rank r  (0 at the within-K reference rank)",
           subtitle="Reference is the largest rank fitted for that K_map",
           x="Rank r", y="log10(R_hat / R_ref)")
    print(p1)
    ...
    p2 <- ggplot(rank_sweep_res, aes(x=rank, y=sf_ratio_ref)) +
      ...
      facet_wrap(~K_map, scales="free_x") +
      theme_minimal() +
      labs(title="Scale-free roughness ratio (sf_hat / sf_ref) vs rank r  (1 at reference rank)",
           subtitle="Reference is the largest rank fitted for that K_map",
           x="Rank r", y="sf_hat / sf_ref")
    print(p2)
  }
}
...
pdf_text_page_12(
  "PART 1 — Latent field maps at t* for each K_map (best MSPE rank per K_map; PICAR; NO SVD)",
  c(
    "For each K_map, we refit the Gaussian PICAR model at r*(K_map) = best MSPE rank.",
    "We reconstruct u_hat(s,t*) on mesh vertices and show one panel per K_map.",
    "All panels share a common color scale for direct visual comparison across K_map."
  )
)
...
for (K_map in K_vals_sweep) {
  ...
  cat("\n[MAP PANELS] Fitting best-rank map for K_map =", K_map, "...\n")
  ...
  M_train <- ST_train_K[, 1:r_best, drop=FALSE]
  MQM_red <- MQM_full_K[1:r_best, 1:r_best, drop=FALSE]
  MQM_red <- 0.5*(MQM_red + t(MQM_red)) + diag(1e-6, nrow(MQM_red))
...
latent_byK$K_label <- paste0("K_map = ", latent_byK$K_map, "   (r* = ", latent_byK$r_best, ")")
...
p_panels <- ggplot(latent_byK, aes(x=lon, y=lat, color=u_hat)) +
  ...
  theme_minimal() +
  labs(
    title    = "Gaussian PICAR (REAL DATA; NO SVD): posterior-mean latent field u_hat(s,t*) at best MSPE rank per K_map",
    subtitle = paste0("t* = ", sprintf("%.4f", t_star), "   |   one panel per K_map (common color scale)"),
...
# Part 2: Mesh-density sensitivity curves
pdf_text_page_12(
  "PART 2 — Mesh-density sensitivity (build time vs mesh density; MCMC time/iter vs mesh density) — PICAR; NO SVD",
  c(
    "We vary mesh density via max.edge0 (larger max.edge0 = coarser mesh).",
    "For each mesh we record:",
    "  (i) one-time basis build time (mesh + A + Moran + ST + MQM)",
    "  (ii) MCMC sampling time per iteration at fixed K_sens and fixed rank",
...
if (nrow(mesh_sens_res) > 0) {
  ...
  p_build <- ggplot(mesh_sens_res, aes(x=N_mesh_vertices, y=basis_build_sec)) +
    geom_line() + geom_point(size=2) +
    theme_minimal() +
    labs(title="One-time basis build time vs mesh density (PICAR; NO SVD)",
         x="Number of mesh vertices", y="Basis build time (sec)")

  p_mcmc <- ggplot(mesh_sens_res, aes(x=N_mesh_vertices, y=mcmc_time_per_iter)) +
    geom_line() + geom_point(size=2) +
    theme_minimal() +
    labs(title="MCMC time per iteration vs mesh density (fixed K_sens, fixed rank; PICAR; NO SVD)",
